...
class ReaderContentBlockList extends ConsumerStatefulWidget {
  const ReaderContentBlockList({
    super.key,
  ...
  @override
  ConsumerState<ReaderContentBlockList> createState() => _ReaderContentBlockListState();
}

class _ReaderContentBlockListState extends ConsumerState<ReaderContentBlockList> {
  ...
  /// Scroll to end of content (for ReaderChapterCompletion)
  ...
  void _scrollToNextBlockAfterActivity(String activityId, List<ContentBlock> visibleBlocks) {
    ...
    if (activityIndex >= visibleBlocks.length - 1) {
      // Last block was an activity, scroll to end marker (ReaderChapterCompletion area)
      _scrollToEndMarker();
      return;
    }
