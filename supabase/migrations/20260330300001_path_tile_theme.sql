-- Add path-level tile_theme_id for unit map background theming.
-- Each learning path (template or scope) can have its own tile theme
-- that controls how the unit map is rendered.

-- 1. Add tile_theme_id to learning path tables
...
-- 2. Update get_user_learning_paths to return path-level tile_theme_id
...
-- 3. Update apply_learning_path_template to copy path-level tile_theme_id
