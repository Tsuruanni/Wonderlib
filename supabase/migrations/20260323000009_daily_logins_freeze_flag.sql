-- Add is_freeze flag to daily_logins and record freeze days in update_user_streak

ALTER TABLE daily_logins ADD COLUMN IF NOT EXISTS is_freeze BOOLEAN DEFAULT false;

-- Recreate update_user_streak to also insert freeze days into daily_logins
...
BEGIN
    ...
    ELSE
        ...
        ELSIF v_freeze_count > 0 THEN
            -- Partial coverage: not enough freezes, streak breaks
            ...
            -- Record the freeze days that were consumed (right after last activity)
