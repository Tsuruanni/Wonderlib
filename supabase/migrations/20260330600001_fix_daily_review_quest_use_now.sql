-- Fix: daily_review quest due word count must use NOW() (not app_now())
-- to match the get_due_review_words RPC that the client calls.
-- app_now() includes debug_date_offset which causes count mismatch.

-- =============================================
-- A. get_daily_quest_progress — use NOW() for daily_review due count
-- =============================================
    ...
    LOOP
        CASE v_quest.quest_type
            WHEN 'daily_review' THEN
                -- Exact same query as get_due_review_words RPC (uses NOW())
                SELECT COUNT(*)::INT
...
-- =============================================
-- B. claim_daily_bonus — same NOW() fix
-- =============================================
