-- =============================================
-- Fix: use daily_logins table instead of non-existent last_login_at column
-- =============================================
...
BEGIN
  ...
  SELECT
    ...
    -- Active: students who have a daily_login in the last 30 days
    COUNT(DISTINCT CASE
      WHEN EXISTS (
        SELECT 1 FROM daily_logins dl
        WHERE dl.user_id = p.id
        AND dl.login_date >= CURRENT_DATE - 30
      ) THEN p.id
