-- Move tile_theme_id from vocabulary_units to learning path unit tables.
-- Tile theme is a learning-path concern, not a vocabulary-unit property.

-- 1. Add tile_theme_id to template units
ALTER TABLE learning_path_template_units
  ADD COLUMN tile_theme_id UUID REFERENCES tile_themes(id) ON DELETE SET NULL;

-- 2. Add tile_theme_id to scope units
ALTER TABLE scope_learning_path_units
  ADD COLUMN tile_theme_id UUID REFERENCES tile_themes(id) ON DELETE SET NULL;

-- 3. Copy existing assignments from vocabulary_units to scope units
UPDATE scope_learning_path_units slpu
SET tile_theme_id = vu.tile_theme_id
FROM vocabulary_units vu
WHERE slpu.unit_id = vu.id AND vu.tile_theme_id IS NOT NULL;

-- 4. Copy existing assignments from vocabulary_units to template units
UPDATE learning_path_template_units lptu
SET tile_theme_id = vu.tile_theme_id
FROM vocabulary_units vu
WHERE lptu.unit_id = vu.id AND vu.tile_theme_id IS NOT NULL;

-- 5. Drop tile_theme_id from vocabulary_units
ALTER TABLE vocabulary_units DROP COLUMN tile_theme_id;

-- 6. Update RPC to read from scope units instead of vocabulary_units
