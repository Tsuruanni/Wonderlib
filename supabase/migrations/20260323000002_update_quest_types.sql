-- Replace "Read 100 words" and "Answer 5 questions" quests with better alternatives
-- "Read 100 words" → unreliable (word_count can be NULL)
-- "Answer 5 questions" → only works if chapters have activities

-- Update existing quests (keeping same IDs for any existing completion records)
UPDATE daily_quests SET
    quest_type = 'read_chapters',
    title = 'Read 3 chapters',
    icon = '📚',
    goal_value = 3,
    reward_type = 'coins',
    reward_amount = 10
WHERE quest_type = 'read_words';

UPDATE daily_quests SET
    quest_type = 'vocab_session',
    title = 'Complete a vocabulary session',
    icon = '🧠',
    goal_value = 1,
    reward_type = 'xp',
    reward_amount = 15
WHERE quest_type = 'correct_answers';

-- Update the RPC to handle new quest types
