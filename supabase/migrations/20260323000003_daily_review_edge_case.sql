-- Fix: daily_review quest auto-completes when no words are due for review
-- Also handle the case in get_daily_quest_progress RPC
    ...
    LOOP
        ...
        CASE v_quest.quest_type
            WHEN 'daily_review' THEN
                ...
                ) INTO v_has_due_words;

                IF NOT v_has_due_words THEN
                    -- No words to review → auto-complete (nothing to do today)
                    v_current := 1;
                ELSE
