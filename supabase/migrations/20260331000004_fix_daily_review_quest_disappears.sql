-- Fix: daily_review quest disappears after completion
-- When a student completes their daily review, word due counts drop below 10
-- (because next_review_at gets pushed forward). The CONTINUE statement was
-- skipping the quest entirely, even though it was already completed.
-- Fix: only CONTINUE if due words < 10 AND no review session exists today.

-- =============================================
-- A. get_daily_quest_progress — keep completed daily_review visible
-- =============================================
    ...
    LOOP
        CASE v_quest.quest_type
            WHEN 'daily_review' THEN
                -- Check if review session exists today FIRST
                ...
                -- Count due words (same query as get_due_review_words RPC)
                SELECT COUNT(*)::INT
                ...
                -- Skip ONLY if not enough due words AND not already completed today
                IF v_due_word_count < 10 AND v_current = 0 THEN
                    CONTINUE;
                END IF;
...
-- =============================================
-- B. claim_daily_bonus — consistent logic for active count
-- =============================================
...
BEGIN
    ...
    -- Check if daily review was completed today
    SELECT EXISTS(
        SELECT 1 FROM daily_review_sessions drs
        WHERE drs.user_id = p_user_id AND drs.session_date = v_today
    ) INTO v_review_done_today;
    ...
    -- Exclude daily_review from active count only if < 10 due words AND not completed today
    SELECT COUNT(*) INTO v_active_count
    FROM daily_quests
    WHERE is_active = true
      AND NOT (quest_type = 'daily_review' AND v_due_word_count < 10 AND NOT v_review_done_today);
