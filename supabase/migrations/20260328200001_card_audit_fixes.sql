...
--
-- Part A (Finding #1): Add image_url to open_card_pack JSONB response
--   The myth_cards.image_url column was already fetched via SELECT mc.*
--   but was not included in the jsonb_build_object result for each card.
--
-- Part B (Finding #4): Add idempotency key to buy_card_pack
--   Adds optional p_idempotency_key UUID parameter. When provided, checks
--   coin_logs for an existing (user_id, 'pack_purchase', source_id) row.
--   If found, returns current state with coins_spent = 0 (no-op).
--   Uses the partial unique index idx_coin_logs_idempotent for conflict safety.

-- =============================================
-- Part A: open_card_pack — include image_url in card JSONB
-- =============================================
...
-- =============================================
-- Part B: buy_card_pack — add idempotency key
-- =============================================
...
BEGIN
    ...
    -- Idempotency check: if key provided and already used, return current state as no-op
    ...
    -- Log coin transaction (include idempotency key as source_id when provided)
