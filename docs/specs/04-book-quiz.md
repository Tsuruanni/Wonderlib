...
| 7 | Dead Code | `BookQuizModel.fromEntity` / `BookQuizQuestionModel.fromEntity` — audit initially marked as unused, but actually called by `book_cache_store.dart` for offline caching. Kept; `_typeToString` replaced with `entity.type.dbValue` from shared enum. | Low | Fixed |
